import Link from "next/link";

import { Compass } from "lucide-react";

import { ROUTES } from "@/constants";

import { WorkspaceNotFoundHarness } from "./not-found.harness";

export default function WorkspaceNotFound() {
	return (
		<div
			data-qa={WorkspaceNotFoundHarness.Root}
			className="bg-surface flex min-h-svh flex-1 flex-col items-center justify-center gap-5 px-6 text-center"
		>
			<span className="bg-brand-muted text-brand-600 dark:text-brand-300 flex size-12 items-center justify-center rounded-xl">
				<Compass className="size-6" />
			</span>

			<div className="flex flex-col gap-2">
				<h1 className="text-text-primary text-xl font-semibold tracking-tight">
					Workspace not found
				</h1>
				<p className="text-text-secondary max-w-sm text-sm">
					This workspace does not exist, or you no longer have access to it. Ask an admin for an
					invite, or switch to one of your own workspaces.
				</p>
			</div>

			<Link
				href={ROUTES.root}
				className="bg-brand-500 hover:bg-brand-600 rounded-md px-4 py-2 text-sm font-medium text-white transition-colors duration-100"
			>
				Go to my workspaces
			</Link>
		</div>
	);
}
